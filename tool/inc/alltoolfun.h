...
/** LOG OUT FUN */
#define BSD_LOG(level, message) \
    LogOutput::getInstance()->logOut(level, \
        QString("(%2)%3: %4").arg(__FUNCTION__).arg(__LINE__).arg(message))

#define BSD_LOG_INFO(message) \
        LogOutput::getInstance()->logOutInfoToView( \
        QString("%4").arg(message))

#define BSD_LOG_GET_LEVEL_NAME(level) LogOutput::getInstance()->packetGetLevelName(level)
#define BSD_LOG_SET_LEVEL(level) LogOutput::getInstance()->logOutputSetLevel(level)
#define BSD_LOG_GET_LEVEL() LogOutput::getInstance()->logOutputGetLevel()
