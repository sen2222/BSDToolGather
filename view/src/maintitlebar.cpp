    ...
    , ui(new Ui::MainTitleBar)
{
    ...
    ui->comboBox->setCurrentIndex(BSD_LOG_GET_LEVEL());
}
...
void MainTitleBar::timeShowSwitch(int showStatus)
{
    ...
    BSD_LOG(LOG_INFO, QString("Time display status changed to %1\n").arg(timeShowStatus));
...
void MainTitleBar::on_comboBox_currentIndexChanged(int index)
{
    if(index >= 0 && index <= 3)
    {
        BSD_LOG_SET_LEVEL(static_cast<LOG_LEVEL_E>(index));
    }
}
